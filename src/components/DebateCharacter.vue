...
import {
  ...
  defaultSpeakingAnimationConfig,
  defaultIdleAnimationConfig,
} from '@/config/characterAnimationConfig.js'
import { getAudio } from '@/services/personaService'

const props = defineProps({
  characterId: { type: String, required: true },
  modelPath: { type: String, required: true },
  renderOverrides: { type: Object, default: null },
})
...
const tick = (time) => {
  ...
  if (isSpeaking.value) {
    ...
    const speakingConfig =
      characterAnimationConfig[props.characterId]?.speaking ?? defaultSpeakingAnimationConfig
    ...
    coreModel.setParameterValueById(
      'ParamEyeLOpen',
      blinkCycle < speakingConfig.blinking.durationMs ? 0 : 1,
    )
    coreModel.setParameterValueById(
      'ParamEyeROpen',
      blinkCycle < speakingConfig.blinking.durationMs ? 0 : 1,
    ...
    coreModel.setParameterValueById(
      'ParamAngleX',
      Math.sin(headTx * 2 * Math.PI) * speakingConfig.head.x.amplitude,
    )
    const headTy = (elapsed % speakingConfig.head.y.cycleMs) / speakingConfig.head.y.cycleMs
    coreModel.setParameterValueById(
      'ParamAngleY',
      Math.cos(headTy * 2 * Math.PI) * speakingConfig.head.y.amplitude,
    )

    const bodyTz = (elapsed % speakingConfig.body.z.cycleMs) / speakingConfig.body.z.cycleMs
    coreModel.setParameterValueById(
      'ParamAngleZ',
      Math.sin(bodyTz * 2 * Math.PI) * speakingConfig.body.z.amplitude,
    )
  } else {
    // --- IDLE ANIMATION LOGIC ---
    const idleConfig =
      characterAnimationConfig[props.characterId]?.idle ?? defaultIdleAnimationConfig
    ...
    coreModel.setParameterValueById(
      'ParamEyeLOpen',
      blinkCycle < idleConfig.blinking.durationMs ? 0 : 1,
    )
    coreModel.setParameterValueById(
      'ParamEyeROpen',
      blinkCycle < idleConfig.blinking.durationMs ? 0 : 1,
    ...
    coreModel.setParameterValueById(
      'ParamAngleZ',
      Math.sin(bodyTz * 2 * Math.PI) * idleConfig.body.z.amplitude,
    )

    // Smoothly return head to neutral
    coreModel.setParameterValueById(
      'ParamAngleX',
      coreModel.getParameterValueById('ParamAngleX') * 0.95,
    )
    coreModel.setParameterValueById(
      'ParamAngleY',
      coreModel.getParameterValueById('ParamAngleY') * 0.95,
    )
  }
...
const loadModel = async () => {
  ...
  try {
    // Disable mipmaps to prevent WebGL errors with 8K textures (Sera)
    // This is a global setting but necessary for stability with large assets
    ...
    // Restore original setting (optional, but good practice)
    PIXI.BaseTexture.defaultOptions.mipmap = originalMipmap

    live2dModel.eventMode = 'none'

    const baseConfig = characterRenderConfig[props.characterId]
    if (baseConfig) {
      // Use overrides if provided, otherwise fallback to base config
      const anchor = props.renderOverrides?.anchor || baseConfig.anchor
      const position = props.renderOverrides?.position || baseConfig.position
      const scale = props.renderOverrides?.scale || baseConfig.scale

      live2dModel.anchor.set(anchor.x, anchor.y)
      live2dModel.x = app.screen.width * position.x
      live2dModel.y = app.screen.height * position.y
      live2dModel.scale.set(scale.x, scale.y)
    }
    ...
    console.log(
      `[DebateCharacter:${props.characterId}] Model loaded and ALL animation loops started.`,
    )
  } catch (error) {
