...
export const characterRenderConfig = {
  aria: {
    anchor: { x: 0.76, y: 0.39 }, // Adjust based on Aria's model pivot
    position: { x: 0.4, y: 1.2 }, // Place Aria on the left side
    scale: { x: 0.64, y: 0.64 }, // Adjust scale for Aria
  ...
  sera: {
    ...
    scale: { x: 0.25, y: 0.25 }, // Adjust scale for Sera
    mouthParam: 'ParamMouthForm', // Example parameter name for lip sync
  },
  eidon: {
    anchor: { x: 0.25, y: 0.65 }, // Adjust based on Eidon's model pivot
    position: { x: 0.75, y: 0.78 }, // Place Eidon on the right side
    scale: { x: 0.06, y: 0.06 }, // Adjust scale for Eidon
